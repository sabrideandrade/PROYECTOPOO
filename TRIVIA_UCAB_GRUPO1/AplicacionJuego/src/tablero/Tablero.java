...
public class Tablero {
    private final List<String> categorias = Arrays.asList("geo", "his", "dep", "cie", "art", "ent");
    ...
    // Método corregido para obtener la categoría según la posición
    public String obtenerCategoria(int posicion) {
        if (posicion >= 0 && posicion < 42) {
            return casillasPerimetro.get(posicion).getCategoria();
        } else if (posicion >= 42 && posicion < 72) {
            int rayo = (posicion - 42) / 5;
            int pos = (posicion - 42) % 5;
            return rayos.get(rayo).get(pos).getCategoria();
        } else if (posicion == 72) {
            return centro.getCategoria();
        } else {
            return "???";
        }
    }
